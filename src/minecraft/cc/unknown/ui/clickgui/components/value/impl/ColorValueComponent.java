...
public class ColorValueComponent extends ValueComponent {
    ...
    public void draw(final Vector2d position, final int mouseX, final int mouseY, final float partialTicks) {
        ...
        final float valueWidth = Fonts.MAIN.get(16, Weight.LIGHT).width(this.value.getName()) + 4;

        Fonts.MAIN.get(16, Weight.LIGHT).draw(this.value.getName(), this.position.x, this.position.y, Colors.SECONDARY_TEXT.getRGB());
...
//                RenderUtil.rectangle(x, y + pickerHeight - 2, pickerWidth - edge * 2, 5, Color.BLACK);
            ...
            Fonts.MAIN.get(17, Weight.LIGHT).drawCentered(color.getRed() + "", textX + padding, textY, Colors.SECONDARY_TEXT.getRGB());
            Fonts.MAIN.get(17, Weight.LIGHT).drawCentered(color.getGreen() + "", textX + 30, textY, Colors.SECONDARY_TEXT.getRGB());
            Fonts.MAIN.get(17, Weight.LIGHT).drawCentered(color.getBlue() + "", textX + padding * 6, textY, Colors.SECONDARY_TEXT.getRGB());

            textY += 13;

            Fonts.MAIN.get(13, Weight.LIGHT).draw(String.format("#%02X%02X%02X", color.getRed(), color.getGreen(), color.getBlue()), textX, textY, new Color(55, 59, 61).hashCode());
        }
    }
