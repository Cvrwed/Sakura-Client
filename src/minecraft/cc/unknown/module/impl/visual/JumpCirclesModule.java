...
@ModuleInfo(aliases = "Jump Circles", description = "Draws circles around the player when they jump", category = Category.VISUALS)
public final class JumpCirclesModule extends Module {
	...
	private void renderCircleOutline(final Circle circle, final Vec3 pos, final double y) {
		...
		for (int i = 0; i <= 360; i++) {
			...
			final Color outerColor = this.getModule(HUD.class).getTheme().getAccentColor(new Vector2d(i, i));
			...
			GL11.glColor4f(outerColor.getRed() / 255F, outerColor.getGreen() / 255F, outerColor.getBlue() / 255F,
					circle.getAlpha() / 255f);
