...
public final class SessionStats extends Module {
	...
	private List<String> killsWords = new ArrayList<>(Arrays.asList("no resistió los ataques", "fue brutalmente asesinado", "ha sido asesinado", "fue empujado al vacío", "ha sido asesinado", "se cayó al vacío", "pensó que era un buen momento de morir a manos", "se ha desconectado intentando correr"));
	...
	public final Listener<MotionEvent> onPreMotionEvent = event -> {
		if (event.isPre()) {
			if (mc.player.ticksExisted % 20 == 0) {
				...
				if (minutes > 0)
					base += minutes + " " + (minutes == 1 ? "minute" : "minutes")
							+ (seconds == 0 || hours > 0 ? "" : " ");
	...
	public final Listener<Render2DEvent> onRender2D = event -> {
		...
		Fonts.MAIN.get(24, Weight.LIGHT).drawCentered("Session Stats", position.position.x + position.scale.x / 2f, position.position.y + padding, getTheme().getAccentColor().getRGB());
		Fonts.MAIN.get(18, Weight.LIGHT).drawCentered(time, position.position.x + position.scale.x / 2f, position.position.y + padding + 19, new Color(255, 255, 255, 200).getRGB());
		Fonts.MAIN.get(18, Weight.LIGHT).drawCentered("Kills:".toLowerCase() + " " + session.kills, position.position.x + 35, position.position.y + padding + 32, new Color(255, 255, 255, 200).getRGB());
		Fonts.MAIN.get(18, Weight.LIGHT).drawWithShadow("Deaths:".toLowerCase() + " " + session.deaths, position.position.x + 75, position.position.y + padding + 32, new Color(255, 255, 255, 200).getRGB());
