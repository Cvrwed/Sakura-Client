...
@ModuleInfo(aliases = "Back Track", description = "Uses latency to give you a reach advantage", category = Category.COMBAT)
public final class BackTrack extends Module {
	...
	private void handlePacket(Packet<?> packet) {
	    ...
	    if (packet instanceof S18PacketEntityTeleport) {
	        Entity entity = mc.world.getEntityByID(((S18PacketEntityTeleport) packet).getEntityId());
	        if (entity instanceof EntityLivingBase) {
	            realPosX = ((S18PacketEntityTeleport) packet).getX();
	            realPosY = ((S18PacketEntityTeleport) packet).getY();
	            realPosZ = ((S18PacketEntityTeleport) packet).getZ();
	        }
	    }
	}
