...
public final class AimAssist extends Module {
	...
	private final ModeValue mode = new ModeValue("Mode", this).add(new SubMode("Basic")).add(new SubMode("Advanced"))
			.setDefault("Advanced");
	...
	private final NumberValue verticalSpeed = new NumberValue("Vertical speed", this, 5, 1, 20, 0.1,
			() -> !aimVertically.getValue() || !mode.is("Basic"));
	private final NumberValue horizontalAimSpeed = new NumberValue("Horizontal Speed", this, 45.0, 5.0, 100.0, 1.0,
			() -> !mode.is("Advanced"));
	private final NumberValue horizontalComplement = new NumberValue("Horizontal Complement", this, 15.0, 2.0, 97.0,
			1.0, () -> !mode.is("Advanced"));
	private final BooleanValue horizontalRandomization = new BooleanValue("Horizontal Random", this, false,
			() -> !mode.is("Advanced"));
	private final NumberValue horizontalRandomizationAmount = new NumberValue("Horizontal Randomization", this, 1.2,
			0.1, 5, 1, () -> !horizontalRandomization.getValue());
	...
	private final BooleanValue aimWhileOnTarget = new BooleanValue("Aim while on target", this, true,
			() -> !mode.is("Basic"));
	...
	private final BooleanValue scoreboardCheckTeam = new BooleanValue("Scoreboard Check Team", this, false,
			() -> !ignoreTeammates.getValue());
	private final BooleanValue checkArmorColor = new BooleanValue("Check Armor Color", this, false,
			() -> !ignoreTeammates.getValue());
	...
	private final BooleanValue mouseOverEntity = new BooleanValue("Mouse Over Entity", this, false,
			() -> !lineOfSightCheck.getValue());
	private final BooleanValue disableAimWhileBreakingBlock = new BooleanValue("Disable While Breaking Blocks", this,
			false);
	...
	public final Listener<PreMotionEvent> onPreMotionEvent = event -> {
		...
		if (mode.is("Basic")) {
			...
			if (targetYaw > curYaw) {
				move = true;
				Vector2f movedRotation = RotationUtil.move(new Vector2f(curYaw, curPitch),
						new Vector2f(targetYaw, curPitch), hSpeed);
			...
			if (aimVertically.getValue()) {
				if (targetPitch > curPitch) {
					move = true;
					Vector2f movedRotation = RotationUtil.move(new Vector2f(curYaw, curPitch),
							new Vector2f(curYaw, targetPitch), vSpeed);
		...
		for (final EntityPlayer entityPlayer : players) {
			if (entityPlayer != mc.player && entityPlayer.deathTime == 0) {
				...
				if (Sakura.instance.getBotManager().contains(entityPlayer) && ignoreBots.getValue())
					continue;
				if (ignoreTeammates.getValue()
						&& PlayerUtil.isTeam(entityPlayer, scoreboardCheckTeam.getValue(), checkArmorColor.getValue()))
					continue;
