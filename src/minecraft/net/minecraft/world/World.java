...
public abstract class World implements IBlockAccess, Accessor, java.io.Serializable {
	...
	public final ConcurrentLinkedQueue<Entity> loadedEntityList = new ConcurrentLinkedQueue<>();	protected final List<Entity> unloadedEntityList = Lists.newArrayList();
	...
	private boolean isAreaLoaded(int xStart, final int yStart, int zStart, int xEnd, final int yEnd, int zEnd,
	...
	public void notifyBlockOfStateChange(final BlockPos pos, final Block blockIn) {
		if (!this.isRemote) {
			...
			try {
				...
				final CrashReport crashreport = CrashReport.makeCrashReport(throwable,
	 ...
	 */
	public MovingObjectPosition rayTraceBlocks(Vec3 vec31, final Vec3 vec32, final boolean stopOnLiquid,
		...
		if (!Double.isNaN(vec31.xCoord) && !Double.isNaN(vec31.yCoord) && !Double.isNaN(vec31.zCoord)) {
			if (!Double.isNaN(vec32.xCoord) && !Double.isNaN(vec32.yCoord) && !Double.isNaN(vec32.zCoord)) {
				...
				if ((!ignoreBlockWithoutBoundingBox
					...
					final MovingObjectPosition movingobjectposition = block.collisionRayTrace(this, blockpos, vec31,
							vec32);
				...
				while (k1-- >= 0) {
					...
					if (!ignoreBlockWithoutBoundingBox
						...
						if (block1.canCollideCheck(iblockstate1, stopOnLiquid)) {
							final MovingObjectPosition movingobjectposition1 = block1.collisionRayTrace(this, blockpos,
									vec31, vec32);
	 ...
	 */
	public void playSoundToNearExcept(final EntityPlayer player, final String name, final float volume,
			final float pitch) {
		for (int i = 0; i < this.worldAccesses.size(); ++i) {
	...
	public void spawnParticle(final EnumParticleTypes particleType, final double xCoord, final double yCoord,
	...
	public void spawnParticle(final EnumParticleTypes particleType, final boolean p_175682_2_, final double xCoord,
	...
	private void spawnParticle(final int particleID, final boolean p_175720_2_, final double xCood, final double yCoord,
			final double zCoord, final double xOffset, final double yOffset, final double zOffset,
			final int... p_175720_15_) {
		for (int i = 0; i < this.worldAccesses.size(); ++i) {
	 ...
	 */
	 public void updateEntities() {
	        ...
	        for (final Entity entity2 : this.loadedEntityList) {
	            if (entity2 == null || mc.player == null) continue;
	        ...
	        for (final Entity entity2 : this.loadedEntityList) {
	            if (entity2 == null || mc.player == null) {
	                continue;
	            }
	            ...
	            if (entity2 == mc.player || entity2.isInvisible()) {
	                entity2.inView = false;
	                continue;
	            }

	            entity2.inView = RayCastUtil.inView(entity2);
	            entity2.threadDistance = mc.player.getDistanceSqToEntity(entity2);
...
//	        });
	 ...
	 * elsewhere.
	 */
	public boolean handleMaterialAcceleration(final AxisAlignedBB bb, final Material materialIn,
				...
				for (int l1 = k; l1 < l; ++l1) {
					for (int i2 = i1; i2 < j1; ++i2) {
						...
						if (block.getMaterial() == materialIn) {
							final double d0 = (float) (l1 + 1) - BlockLiquid
	 ...
	 */
	public Explosion createExplosion(final Entity entityIn, final double x, final double y, final double z,
			final float strength, final boolean isSmoking) {
	 ...
	 * finished)
	 */
	public Explosion newExplosion(final Entity entityIn, final double x, final double y, final double z,
			final float strength, final boolean isFlaming, final boolean isSmoking) {
	...
	protected void playMoodSoundAndCheckLight(final int p_147467_1_, final int p_147467_2_, final Chunk chunkIn) {
		...
		if (this.ambientTickCountdown == 0 && !this.isRemote) {
			...
			if (block.getMaterial() == Material.air && this.getLight(blockpos) <= this.rand.nextInt(8)
				...
				final EntityPlayer entityplayer = this.getClosestPlayer((double) j + 0.5D, (double) l + 0.5D,
						(double) k + 0.5D, 8.0D);
		...
		if (f > 0.15F) {
			...
			if (pos.getY() >= 0 && pos.getY() < 256 && this.getLightFor(EnumSkyBlock.BLOCK, pos) < 10) {
				...
				if ((block == Blocks.water || block == Blocks.flowing_water)
					...
					final boolean flag = this.isWater(pos.west()) && this.isWater(pos.east())
							&& this.isWater(pos.north()) && this.isWater(pos.south());
	...
	public <T extends Entity> List<T> getPlayers(final Class<? extends T> playerType,
			final Predicate<? super T> filter) {
		final List<T> list = Lists.newArrayList();
	...
	public <T extends Entity> List<T> getEntitiesWithinAABB(final Class<? extends T> classEntity,
	...
	public <T extends Entity> T findNearestEntityWithinAABB(final Class<? extends T> entityType,
			final AxisAlignedBB aabb, final T closestTo) {
	...
	public boolean canLightningStrike(final BlockPos strikePosition) {
		if (!this.isRaining()) {
			...
			return !biomegenbase.getEnableSnow()
					&& (!this.canSnowAt(strikePosition, false) && biomegenbase.canSpawnLightningBolt());
		}
	}
	...
	public void playAuxSFXAtEntity(final EntityPlayer player, final int sfxType, final BlockPos pos,
	...
	public Random setRandomSeed(final int p_72843_1_, final int p_72843_2_, final int p_72843_3_) {
		final long i = (long) p_72843_1_ * 341873128712L + (long) p_72843_2_ * 132897987541L
