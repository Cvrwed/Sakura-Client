...
public class EntityPlayerSP extends AbstractClientPlayer implements Accessor {
    ...
    public void onUpdateWalkingPlayer() {
        final MotionEvent event = new MotionEvent(
                ...
                this.rotationPitch,
                this.onGround,
                this.isSprinting(), MotionEvent.Type.Pre
        );
...
//            Rise.INSTANCE.getEventBus().handle(minimumFlyingEvent);
    ...
    protected boolean pushOutOfBlocks(final double x, final double y, final double z) {
        if (this.noClip) {
            ...
            if (!this.isOpenBlockSpace(blockpos)) {
                ...
                Sakura.instance.getEventBus().handle(pushOutOfBlockEvent);
    ...
    public void onLivingUpdate() {    	
        ...
        Sakura.instance.getEventBus().handle(slowDownEvent);
        ...
        if (this.onGround && !flag1 && !flag2 && (this.omniSprint || this.movementInput.moveForward >= f) && !isSprinting() && flag3 && (!isUsingItem() || slowDownEvent.isSprint()) && !isPotionActive(Potion.blindness)) {
        ...
        if (!isSprinting() && (this.omniSprint || this.movementInput.moveForward >= f) && flag3 && (!isUsingItem() || slowDownEvent.isSprint()) && !isPotionActive(Potion.blindness) && this.mc.gameSettings.keyBindSprint.isKeyDown()) setSprinting(true); 
        
        if (!slowDownEvent.isSprint() && isUsingItem() && !isRiding()) setSprinting(false); 

        if (this.isSprinting() && (!this.omniSprint && (this.movementInput.moveForward < f || this.isCollidedHorizontally || !flag3))) this.setSprinting(false);
