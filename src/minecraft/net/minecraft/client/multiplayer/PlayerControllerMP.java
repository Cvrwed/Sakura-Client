        ...
        if (!this.mc.world.getWorldBorder().contains(loc)) {
            ...
            if (this.currentGameType.isCreative()) {
                ...
                if (mc.objectMouseOver != null && mc.objectMouseOver.typeOfHit.equals(MovingObjectPosition.MovingObjectType.BLOCK) && mc.objectMouseOver.getBlockPos().equals(loc)) {
                    final BlockDamageEvent bdEvent = new BlockDamageEvent(this.mc.player, this.mc.player.worldObj, loc);
                    Sakura.instance.getEventBus().handle(bdEvent);
                }

                if (flag && block1.getPlayerRelativeBlockHardness(mc.player, this.mc.player.worldObj, loc) >= 1.0F) {
