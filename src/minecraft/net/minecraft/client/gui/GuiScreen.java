...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback, Accessor {
	...
	public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
		// Do not replace with an enhanced for loop this can cause null pointer
		// exceptions
	...
	protected void drawHoveringText(List<String> textLines, int x, int y) {
		if (!textLines.isEmpty()) {
			...
			for (String s : textLines) {
				int j = this.fontRendererObj.width(s);
		...
		if (component != null && component.getChatStyle().getChatHoverEvent() != null) {
			...
			if (hoverevent.getAction() == HoverEvent.Action.SHOW_ITEM) {
				...
				if (itemstack != null) {
					...
					this.drawCreativeTabHoveringText(ChatFormatting.RED + "Invalid Item!", x, y);
				}
				...
				if (this.mc.gameSettings.advancedItemTooltips) {
					try {
						...
						if (nbtbase1 instanceof NBTTagCompound) {
							...
							this.drawCreativeTabHoveringText(ChatFormatting.RED + "Invalid Entity!", x, y);
						}
				...
				if (statbase != null) {
					...
					this.drawCreativeTabHoveringText(ChatFormatting.RED + "Invalid statistic/achievement!", x, y);
				}
			}
	...
	protected void mouseReleased(final int mouseX, final int mouseY, final int state) {
		...
		Sakura.instance.getEventBus().handle(guiMouseReleaseEvent);
	...
	public void handleKeyboardInput() throws IOException {
		if (Keyboard.getEventKeyState()) {
			...
			Sakura.instance.getEventBus()
					.handle(new GuiKeyBoardEvent(Keyboard.getEventKey(), Keyboard.getEventCharacter(), this));
		}
